...
export async function interactivityHandler({ render, request }: AppContext) {
  return render(
    <InteractivityPage requestUrl={request.url} />,
    docsResponseInit,
  );
}
...
function InteractivityPage(handle: Handle<DocsPageProps>) {
  return () => (
    ...
    >
      ...
      <DocsSection
        id="refs-attrs-and-dom-lifecycle"
        title="Refs, attrs, and DOM lifecycle"
      >
      ...
      >
        <p>
          Placeholder for Coordinating forms, fetches, frame reloads, and
          navigation.
        </p>
      </DocsSection>
    </DocsChapter>
