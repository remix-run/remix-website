...
```ts
export let loader = ({ session }) => {};
export let action = ({ session }) => {};
```
...
```ts
export let action = async ({ params, session }) => {
...
Destroys a session object. This is usually useful when logging someone out of
your site.
