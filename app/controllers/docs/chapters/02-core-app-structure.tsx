...
export async function coreAppStructureHandler({ render, request }: AppContext) {
  return render(
    <CoreAppStructurePage requestUrl={request.url} />,
    docsResponseInit,
  );
}
...
function CoreAppStructurePage(handle: Handle<DocsPageProps>) {
  return () => (
    ...
    >
      ...
      >
        <p>
          Placeholder for Route builders: route, get, post, put, del, form,
          resources.
        </p>
      </DocsSection>
      ...
      <DocsSection
        id="nested-route-maps-and-ownership"
        title="Nested route maps and ownership"
      >
      ...
      <DocsSection
        id="app-organization-patterns"
        title="App organization patterns"
      >
