...
## `app/global-data.{js,ts}`

This data module is called on the initial server render and after data actions in the client. It's useful for getting data to the App component or global things like the user's language preference or (public) server environment variables that the browser JavaScript needs to initialize (like Stripe public keys, etc).
...
When a route is not found, or an error is unhandled, we use these routes. We don't like this convention because "/404" should be a valid URL (maybe you're showing the best BBQ in Atlanta!). This is what we've got right now though. This will change.
