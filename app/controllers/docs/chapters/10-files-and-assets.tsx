...
export async function filesAndAssetsHandler({ render, request }: AppContext) {
  return render(
    <FilesAndAssetsPage requestUrl={request.url} />,
    docsResponseInit,
  );
}
...
function FilesAndAssetsPage(handle: Handle<DocsPageProps>) {
  return () => (
    ...
    >
      ...
      <DocsSection
        id="remix-s-unbundled-asset-server"
        title="Remix's unbundled asset server"
      >
      ...
      >
        <p>
          Placeholder for File maps, allow and deny rules, and browser-only
          modules.
        </p>
      </DocsSection>
      ...
      >
        <p>
          Placeholder for File downloads, lazy files, MIME types, and range
          responses.
        </p>
      </DocsSection>
    </DocsChapter>
