...
describe("home route", () => {
  it("renders the landing document shell", async () => {
    ...
    expect(html).toContain(
      '<link rel="modulepreload" href="/assets/app/actions/public/entry.ts"',
    );
    expect(html).toContain(
      '<script type="module" async src="/assets/app/actions/public/entry.ts"',
    );

    expect(html).toMatch(/class="loading-screen-overlay\b/);
    expect(html.indexOf('id="remix-landing-app"')).toBeLessThan(
      html.indexOf("loading-screen-overlay"),
    );
