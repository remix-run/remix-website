...
You've seen a `loader`. Now you're going to create an `action`. Every data route can export a "reader" and a "writer". Just like React, you have `[state, setState] = React.useState()`, or `[state, dispatch] = React.useReducer()`. Readers and writers. Data and mutations. _Loaders and Actions_.
