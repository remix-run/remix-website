...
- Use `staticFiles()` for files that should be served directly from disk, such as images, fonts,
  or already-built assets in the root `public/` directory
...
- A `public/` directory inside `app/` marks browser-reachable source for the asset server; the
  root `public/` directory is the separate home for files served unchanged
