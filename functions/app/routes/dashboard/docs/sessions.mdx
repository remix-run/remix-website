...
Use the session storage object to get access sessions in your `loader` and/or `action` function (see <Link to="../mutations">mutations</Link>).

The `getSession()` function retrieves the current session from the incoming request's `Cookie` header, and `commitSession()`/`destroySession()` provide the `Set-Cookie` header for the outgoing response.
