...
export async function authSessionsSecurityHandler({
  render,
  request,
}: AppContext) {
  return render(
    <AuthSessionsSecurityPage requestUrl={request.url} />,
    docsResponseInit,
  );
}
...
function AuthSessionsSecurityPage(handle: Handle<DocsPageProps>) {
  return () => (
    ...
    >
      ...
      <DocsSection
        id="regenerating-session-ids"
        title="Regenerating session IDs"
      >
      ...
      <DocsSection
        id="route-protection-with-requireauth"
        title="Route protection with requireAuth"
      >
