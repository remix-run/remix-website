...
<!-- Diagrams, exported as PNG @ 2x scale: https://excalidraw.com/#json=V90fQLt2mHxQnRuXn-5hS,pBKDQqcOAiKfXZf4bTRh1g -->
...
<img alt="Waterfall diagram showing `Click /route` triggering `Get route module` (split into `clientLoader` and `Component` segments) followed by `Run clientLoader` before `Render content` occurs" src="/blog-images/posts/split-route-modules/get-route-module.png" class="md-diagram md-diagram-centered" />
...
<img alt="Waterfall diagram showing `Click /route` triggering a parallel `Get clientLoader` and `Get Component` followed by `Run clientLoader` before `Render content` occurs, now earlier than before" src="/blog-images/posts/split-route-modules/get-route-module-split.png" class="md-diagram md-diagram-centered" />
